class Converter {
    ...
    async getHistory(historyDir){
        ...
        for (let path of records){
            result = await parseHistoryDir(path);
            current.push(result);
        }
        console.log(current);
    }    
    ...
    async parseDir(filepath){
        ...
        if (filename == 'collection.nml'){
            created = moment();
        } else {
            d = filename.match(/collection_(?<year>\d{4})y(?<month>\d{2})m(?<day>\d{1,2})d_(?<hour>\d{1,2})h(?<minute>\d{1,2})m(?<second>\d{1,2}).*/).groups;
            created = moment().parseZone(`${d.year}-${d.month}-${d.day} ${d.hour}:${d.minute}:${d.second}-08:00`);
        }
